var jsPsychInstructionsMyVersion = (function (jspsych) {
    'use strict';

    const info = {
        name: "instructionsMyVersion",
        parameters: {
     ...
     * Additionally, makes it possible the following:
     * 1) Not allow participants to move forward until they have spent a certain amount of time on the page.
     * 2) Not allow participannts to move forwards until they watched the video/videos present on that screen.
     *
     * @author Fatih Sogukpinar
     * @see {@link https:/?? instructions plugin documentation jspsych + my github}
     */
    class InstructionsPluginMyVersion {
        constructor(jsPsych) {
        ...
        trial(display_element, trial) {
            ...
            var viewedPages = [];
            var pageMinTimeTimer = null;
            var canAdvanceForward = false;
            var isNavigating = false;
            var keyboard_listener = null;

            function clearPageTimers() {
                if (pageMinTimeTimer !== null) {
                    clearTimeout(pageMinTimeTimer);
                    pageMinTimeTimer = null;
                }
            }

            function pausePageVideos() {
                display_element.querySelectorAll("video").forEach(function (video) {
                    video.pause();
                });
            }
            ...
            function show_current_page() {
                ...
                var pageData = trial.pages[current_page];
                var html = pageData.content;
                var pagenum_display = "";
                var minimum_viewing_time = pageData.minTime;
                var pageAlreadyViewed = viewedPages.includes(current_page);
                canAdvanceForward = pageAlreadyViewed;

                if (!pageAlreadyViewed) {
                    viewedPages.push(current_page);
                }
                ...
                if (trial.show_clickable_nav) {
                    ...
                    var backButton = display_element.querySelector("#jspsych-instructions-back");
                    nextButton.disabled = !canAdvanceForward;

                    if (typeof setupInstructionVideos === "function") {
                        setupInstructionVideos(display_element);
                    }

                    var syncGrid = display_element.querySelector("[data-instruction-video-sync]");
                    var videoElements = display_element.querySelectorAll("video");
                    var videosEnded = 0;
                    var syncVideosComplete = pageAlreadyViewed;
                    var minTimePassed = pageAlreadyViewed;

                    function tryEnableNext() {
                        if (canAdvanceForward) {
                            return;
                        }
                        var videosOk = videoElements.length === 0;
                        if (!videosOk) {
                            videosOk = syncGrid ? syncVideosComplete : videosEnded >= videoElements.length;
                        }
                        if (minTimePassed && videosOk) {
                    ...
                    if (!pageAlreadyViewed) {
                        pageMinTimeTimer = setTimeout(function () {
                            minTimePassed = true;
                            tryEnableNext();
                        }, minimum_viewing_time);
                    }

                    if (syncGrid && typeof setupInstructionSyncedVideoGrid === "function") {
                        setupInstructionSyncedVideoGrid(display_element, function () {
                            syncVideosComplete = true;
                            tryEnableNext();
                        });
                    ...
                    if (current_page !== 0 && trial.allow_backward && backButton) {
                        backButton.addEventListener("click", btnListener);
                    }
                    nextButton.addEventListener("click", btnListener);
            ...
            const endTrial = () => {
                clearPageTimers();
                pausePageVideos();
                if (trial.allow_keys && keyboard_listener) {
    ...
    InstructionsPluginMyVersion.info = info;

    return InstructionsPluginMyVersion;
